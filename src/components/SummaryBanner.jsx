...
const VERDICTS = {
  strong:  { label: 'STRONG',        sub: 'Financial Health',   color: '#16a34a', ring: '#16a34a', shadow: 'rgba(22,163,74,0.22)'   },
  moderate:{ label: 'MODERATE',      sub: 'Financial Health',   color: '#d97706', ring: '#f59e0b', shadow: 'rgba(245,158,11,0.22)'  },
  below:   { label: 'BELOW AVERAGE', sub: 'Requires Attention', color: '#2563eb', ring: '#2563eb', shadow: 'rgba(37,99,235,0.24)'  },
  critical:{ label: 'CRITICAL',      sub: 'Immediate Action',   color: '#dc2626', ring: '#ef4444', shadow: 'rgba(239,68,68,0.2)'   },
};
  ...
  return (
    <div className="relative w-32 h-32 flex-shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="mono text-[10px] mt-0.5" style={{ color: 'var(--text-5)' }}>/100</span>
      </div>
    </div>
  );
}
...
export default function SummaryBanner({ statuses, onExportPDF, onExportExcel, exporting, onShare, shareCopied, isSharedView, companyContext }) {
  ...
  return (
    <div className="ghost-card rounded-2xl mb-8 overflow-hidden relative animate-in"
      ...
      {/* Ambient glow blobs */}
      <div className="h-[2px]" style={{ background: 'linear-gradient(90deg,#ef4444,#22c55e,#3b82f6)' }} />
      ...
      <div className="relative flex flex-col lg:flex-row">
        ...
        <div className="flex-1 p-6">
          ...
          <div className="flex items-center gap-2 mb-5">
            <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: v.ring }} />
            <span className="mono text-[10px] font-semibold uppercase tracking-[0.14em]"
              style={{ color: 'var(--text-4)' }}>
              Valoreva Score · {new Date().toLocaleDateString('en-IN')}
            </span>
          </div>
          ...
          <div className="flex items-center gap-6 mb-6">
            ...
            <div>
              ...
              <div className="text-[14px] font-medium mt-1" style={{ color: 'var(--text-2)' }}>
                {v.sub}
              </div>
              <div className="mono text-[11px] mt-2" style={{ color: 'var(--text-4)' }}>
          ...
          {isSharedView && (
            <div className="flex items-center gap-2 mb-4 px-3 py-2 rounded-lg text-xs"
              style={{ background: 'rgba(79,110,247,0.08)', border: '1px solid rgba(79,110,247,0.22)', color: 'var(--gold-hi)' }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
                <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/>
                <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/>
              </svg>
              <span>Shared analysis — <a href="/dashboard" className="underline" style={{ color: 'var(--gold-hi)' }}>run your own</a></span>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={onExportPDF}
              ...
              onMouseEnter={e => { e.currentTarget.style.background='var(--surface-hi)'; e.currentTarget.style.borderColor='var(--gold)'; e.currentTarget.style.color='var(--text-1)'; }}
            ...
            {onShare && (
              <button onClick={onShare}
                ...
                style={{
                  ...
                  color: shareCopied ? '#00e887' : 'var(--gold-hi)',
                }}
            ...
            {ticker && (
              <>
                <button
                  ...
                  className="inline-flex items-center gap-2 text-[12px] font-semibold px-4 py-2.5 rounded-xl transition-all duration-200"
                  style={{
                background: (watchDone || onWatchlist) ? 'rgba(37,99,235,0.12)' : 'var(--surface)',
                border: (watchDone || onWatchlist) ? '1px solid rgba(37,99,235,0.35)' : '1px solid var(--border)',
                color: (watchDone || onWatchlist) ? '#2563eb' : 'var(--text-3)',
